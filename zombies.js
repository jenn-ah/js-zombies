...
class Player {
  ...
  takeItem(item) {
    if (this._pack.length < 3) {
      ...
      console.log(`${this.name} packed ${item}`); //should be item.name
      return true;
    } else {
      console.log(`Pack is full so ${item} cannot be stored`); //should be item.name
      return false;
    }
  }
  ...
  discardItem(item) {
    if (this._pack.indexOf(item) !== -1) {
      ...
      console.log(`${this.name} discarded ${item}`); //should be item.name
      return true;
    } else {
      console.log(`Nothing was discarded since ${item} could not be found.`) //should be item.name
      return false;
    }
  }
  ...
  equip(itemToEquip) {
    //if (!itemToEquip instanceof Weapon) { return; }; validation

    if (this.equipped === Item) { //use instanceof Weapon; not needed if valdation
      this.equipped = itemToEquip;
    } else if (this._pack.includes(itemToEquip) === true) { //use instanceof Weapon also if no validation
   ...
   */
  equippedWith() {
    if (this.equipped === false) { //false should be last
